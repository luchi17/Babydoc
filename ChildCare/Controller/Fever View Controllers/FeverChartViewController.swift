...
class FeverChartViewController : UIViewController, ChartViewDelegate, ChartDelegate{
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if selectedMonth == Date().month{
            ...
            if self.childApp!.name.isEmpty && self.registeredChildren!.count > 0{
                ...
                let controller = UIAlertController(title: nil, message: "There are no children in ChildCare.", preferredStyle: .alert)
            ...
            else if !self.childApp!.name.isEmpty && self.childApp!.fever.count == 0{
    ...
    func didTouchChart(_ chart: Chart, indexes: [Int?], x: Double, left: CGFloat) {
        ...
        if !self.childApp!.name.isEmpty && self.childApp!.fever.count != 0{
    ...
    func loadChildren(){
        ...
        if !(childApp?.name.isEmpty)! && childApp?.fever.count != 0{
