...
class VaccineViewController : UIViewController{
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        DispatchQueue.main.async {
            ...
            if self.registeredChildren!.count > 0 && !self.childApp.name.isEmpty && self.childApp.dateOfBirth.isEmpty{
                ...
                let controller = UIAlertController(title: nil, message: "The recommended dates for the vaccines' administration could not be calculated, please enter the date of birth of \(self.childApp.name)", preferredStyle: .alert)
    ...
    func loadChildrenAndVaccines(){
        ...
        if !childApp.name.isEmpty{
            vaccines = childApp.vaccines.filter(NSPredicate(value: true))
    ...
    func configureDictionariesOfVaccine(){
        ...
        for string  in sectionAgeFunded{
            ...
            let vaccinesForAgeFunded = vaccines?.filter(
                "ANY doses.ageOfVaccination == %@ AND funded == %@"
                , string, true)
...
extension VaccineViewController : UITableViewDataSource, UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        ...
        else if registeredChildren!.count > 0 && childApp.dateOfBirth.isEmpty{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 0:
            ...
            if registeredChildren?.count == 0{
                cell.name.text = "No children added yet"
                cell.dateAdministration.text = ""
                cell.accessoryType = .none
            }
            else if childApp.name.isEmpty{
                cell.name.text = "There are no active children"
                cell.dateAdministration.text = ""
                cell.accessoryType = .none
            }
            else{
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath, for orientation: SwipeActionsOrientation) -> [SwipeAction]? {
        ...
        if orientation == .left{
            ...
            case 0 :
                ...
                currentChildSwipeAction.backgroundColor = UIColor.init(hexString: "F78BF9")
                currentChildSwipeAction.hidesWhenSelected = true
                currentChildSwipeAction.image = UIImage(named: "checkmark2")!
